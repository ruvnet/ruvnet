...
import { CATALOG, UPSTREAM, VERSION, HOSTS, CHANGE_PROJECTS, CHANGE_KINDS, discover, project, changes, searchChanges, plan, connect } from './core.mjs';
...
export const TOOLS = [
  ...
  { name: 'ruvnet_changes', description: 'Read and optionally filter the reviewed upstream change snapshot by project or kind. Follow nextCursor with the returned snapshotId; a changed snapshot fails closed so hosts can restart without mixing evidence.', inputSchema: schema({ limit: { type: 'integer', minimum: 1, maximum: 20 }, project: { type: 'string', enum: CHANGE_PROJECTS }, kind: { type: 'string', enum: CHANGE_KINDS }, cursor: { type: 'string', minLength: 1, maxLength: 96, pattern: '^v1\\.[A-Za-z0-9_-]{16}\\.[0-9a-z]+$' }, snapshotId: { type: 'string', minLength: 71, maxLength: 71, pattern: '^sha256:[a-f0-9]{64}$' } }) },
  { name: 'ruvnet_search_changes', description: 'Search and page one reviewed snapshot by exact token overlap. Continue only with the returned snapshot- and query-bound cursor. Raw relevance is not semantic similarity, answer confidence, ranking confidence or utility.', inputSchema: { ...schema({ query: { type: 'string', minLength: 1, maxLength: 200 }, limit: { type: 'integer', minimum: 1, maximum: 20 }, minRawRelevance: { type: 'number', minimum: 0, maximum: 1 }, project: { type: 'string', enum: CHANGE_PROJECTS }, kind: { type: 'string', enum: CHANGE_KINDS }, cursor: { type: 'string', minLength: 1, maxLength: 96, pattern: '^s1\\.[A-Za-z0-9_-]{16}\\.[0-9a-z]+$' }, snapshotId: { type: 'string', minLength: 71, maxLength: 71, pattern: '^sha256:[a-f0-9]{64}$' } }), required: ['query'] } },
...
export function dispatch(name, args = {}) {
  ...
  if (name === 'ruvnet_changes') return changes(args.limit, Date.now(), { ...(args.project === undefined ? {} : { project: args.project }), ...(args.kind === undefined ? {} : { kind: args.kind }) }, args.cursor, args.snapshotId);
  if (name === 'ruvnet_search_changes') return searchChanges(args.query, args.limit, { ...(args.project === undefined ? {} : { project: args.project }), ...(args.kind === undefined ? {} : { kind: args.kind }) }, Date.now(), args.minRawRelevance, args.cursor, args.snapshotId);
