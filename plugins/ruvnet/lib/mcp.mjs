...
export const TOOLS = [
  ...
  { name: 'ruvnet_search_changes', description: 'Search reviewed upstream changes by exact token overlap. Returns matched terms and a raw overlap fraction; this is not semantic similarity or confidence.', inputSchema: { ...schema({ query: { type: 'string', minLength: 1, maxLength: 200 }, limit: { type: 'integer', minimum: 1, maximum: 20 }, project: { type: 'string', enum: CHANGE_PROJECTS }, kind: { type: 'string', enum: CHANGE_KINDS } }), required: ['query'] } },
...
export function dispatch(name, args = {}) {
  ...
  if (name === 'ruvnet_search_changes') return searchChanges(args.query, args.limit, { ...(args.project === undefined ? {} : { project: args.project }), ...(args.kind === undefined ? {} : { kind: args.kind }) });
