...
| `ruvnet_changes` | Read the reviewed upstream snapshot with exact revisions, evidence links and `current`, `stale` or `clock-skew` freshness state. |
...
Resources: `ruv://catalog`, `ruv://changes`, `ruv://federation`. Prompt: `ruvnet-start` with a `goal` argument. The local process has no HTTP listener, network tools, shell execution or mutable memory. The changes resource is a reviewed snapshot rather than a live feed. Inspect `freshness.state` before relying on it; `stale` begins at 72 hours and `clock-skew` means the observation appears to be in the future. A remote-only host cannot connect directly to local stdio.
