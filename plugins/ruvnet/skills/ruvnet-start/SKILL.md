...
Before using upstream-change guidance, call `ruvnet_changes` and inspect `freshness.state`. Use its exact `project` and `kind` filters when the full snapshot is unnecessary. Refresh provenance rather than relying on a `stale` or `clock-skew` snapshot; an empty filtered result means no reviewed match, not that upstream has no matching change.
