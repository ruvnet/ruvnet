...
Before using upstream-change guidance, call `ruvnet_changes` and inspect `freshness.state`. Use its exact `project` and `kind` filters when the full snapshot is unnecessary. Follow `nextCursor` with the same filters until it is absent; treat the cursor as opaque and start over if it is rejected because the reviewed snapshot changed. Refresh provenance rather than relying on a `stale` or `clock-skew` snapshot; an empty filtered result means no reviewed match, not that upstream has no matching change.
