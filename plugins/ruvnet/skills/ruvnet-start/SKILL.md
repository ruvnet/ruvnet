...
Before using upstream-change guidance, call `ruvnet_changes` and inspect `freshness.state`. Use its exact `project` and `kind` filters when the full snapshot is unnecessary. Follow `nextCursor` with the same filters and returned `snapshotId` until the cursor is absent. Treat both as opaque; if the snapshot precondition is rejected, discard cached pages and restart without either value. Refresh provenance rather than relying on a `stale` or `clock-skew` snapshot; an empty filtered result means no reviewed match, not that upstream has no matching change.
