...
Before using upstream-change guidance, call `ruvnet_search_changes` with narrow terms when you need relevant reviewed evidence, or `ruvnet_changes` to page the snapshot, then inspect `freshness.state`. Search uses exact token overlap and exposes matched terms plus a raw overlap fraction; never present that fraction as semantic similarity, confidence, or proof of capability. Use exact `project` and `kind` filters when appropriate. Follow `nextCursor` with the same filters and returned `snapshotId` until the cursor is absent. Treat both as opaque; if the snapshot precondition is rejected, discard cached pages and restart without either value. Refresh provenance rather than relying on a `stale` or `clock-skew` snapshot; an empty result means no reviewed exact-token match, not that upstream has no matching change.
