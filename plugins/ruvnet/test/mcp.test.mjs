...
async function exercise(c){
 const t=await c.listTools();assert.equal(t.tools.length,5);assert.ok(t.tools.every(x=>x.annotations.readOnlyHint&&x.annotations.destructiveHint===false));const changesTool=t.tools.find(x=>x.name==='ruvnet_changes');assert.ok(changesTool.inputSchema.properties.cursor);assert.equal(changesTool.inputSchema.properties.cursor.maxLength,96);
 for(const [name,args] of [['ruvnet_discover',{query:'wifi'}],['ruvnet_project',{id:'ruflo'}],['ruvnet_changes',{limit:2}],['ruvnet_changes',{limit:2,project:'ruflo',kind:'security'}],['ruvnet_plan',{goal:'build harness'}],['ruvnet_connect',{host:'chatgpt'}]]){const r=await c.callTool({name,arguments:args});assert.ok(!r.isError);assert.doesNotThrow(()=>JSON.parse(r.content[0].text));}
 ...
 const first=JSON.parse((await c.callTool({name:'ruvnet_changes',arguments:{limit:2,project:'ruflo',kind:'security'}})).content[0].text);const second=JSON.parse((await c.callTool({name:'ruvnet_changes',arguments:{limit:2,project:'ruflo',kind:'security',cursor:first.nextCursor}})).content[0].text);assert.equal(second.offset,2);assert.equal(new Set([...first.changes,...second.changes].map(change=>change.revision)).size,4);
 for(const [name,args] of [['ruvnet_project',{id:'../../etc/passwd'}],['ruvnet_changes',{limit:21}],['ruvnet_changes',{project:'unknown'}],['ruvnet_changes',{kind:'unknown'}],['ruvnet_changes',{limit:2,cursor:first.nextCursor}],['ruvnet_changes',{cursor:'v1.AAAAAAAAAAAAAAAA.1'}],['ruvnet_connect',{host:'evil'}],['ruvnet_plan',{}],['ruvnet_discover',{execute:true}],['publish',{}]])assert.equal((await c.callTool({name,arguments:args})).isError,true);
...
test('CLI success and invalid-command exit codes',()=>{
 for(const args of [[],['--help'],['--version'],['catalog','memory'],['project','ruflo'],['changes'],['changes','2'],['changes','2','ruflo','security'],['plan','harness'],['connect','stdio']]){const r=spawnSync(process.execPath,[bin,...args],{encoding:'utf8',timeout:5000});assert.equal(r.status,0);assert.equal(r.stderr,'');assert.ok(r.stdout);}
 for(const args of [['nonsense'],['mcp','extra'],['project'],['changes','21'],['changes','2','unknown'],['changes','2','ruflo','unknown'],['changes','2','ruflo','security','extra'],['connect','evil'],['plan']]){const r=spawnSync(process.execPath,[bin,...args],{encoding:'utf8',timeout:5000});assert.equal(r.status,1);assert.equal(r.stdout,'');assert.ok(r.stderr);}
 const first=spawnSync(process.execPath,[bin,'changes','2','ruflo','security'],{encoding:'utf8',timeout:5000});const page=JSON.parse(first.stdout);const next=spawnSync(process.execPath,[bin,'changes','2','ruflo','security',page.nextCursor],{encoding:'utf8',timeout:5000});assert.equal(next.status,0);assert.equal(JSON.parse(next.stdout).offset,2);
});
