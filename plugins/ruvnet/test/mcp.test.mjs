...
async function exercise(c){
 const t=await c.listTools();assert.equal(t.tools.length,4);assert.ok(t.tools.every(x=>x.annotations.readOnlyHint));
 for(const [name,args] of [['ruvnet_discover',{query:'wifi'}],['ruvnet_project',{id:'ruflo'}],['ruvnet_plan',{goal:'build harness'}],['ruvnet_connect',{host:'chatgpt'}]]){const r=await c.callTool({name,arguments:args});assert.ok(!r.isError);assert.doesNotThrow(()=>JSON.parse(r.content[0].text));}
 for(const [name,args] of [['ruvnet_project',{id:'../../etc/passwd'}],['ruvnet_connect',{host:'evil'}],['ruvnet_plan',{}],['ruvnet_discover',{execute:true}],['publish',{}]])assert.equal((await c.callTool({name,arguments:args})).isError,true);
 const rs=await c.listResources();assert.equal(rs.resources.length,2);for(const r of rs.resources)assert.ok((await c.readResource({uri:r.uri})).contents.length);
...
test('CLI success and invalid-command exit codes',()=>{
 for(const args of [[],['--help'],['--version'],['catalog','memory'],['project','ruflo'],['plan','harness'],['connect','stdio']]){const r=spawnSync(process.execPath,[bin,...args],{encoding:'utf8',timeout:5000});assert.equal(r.status,0);assert.equal(r.stderr,'');assert.ok(r.stdout);}
 for(const args of [['nonsense'],['mcp','extra'],['project'],['connect','evil'],['plan']]){const r=spawnSync(process.execPath,[bin,...args],{encoding:'utf8',timeout:5000});assert.equal(r.status,1);assert.equal(r.stdout,'');assert.ok(r.stderr);}
});
