...
import {discover,project,plan,connect,HOSTS} from '../lib/core.mjs';
...
test('all supported hosts and dispatch paths',()=>{
 ...
 assert.equal(dispatch('ruvnet_project',{id:'ruflo'}).id,'ruflo');assert.equal(dispatch('ruvnet_connect',{host:'lovable'}).host,'lovable');assert.equal(dispatch('ruvnet_plan',{goal:'harness'}).executed,false);assert.ok(dispatch('ruvnet_discover').matches.length);
});
